...
/**
 * AskIan – lightweight chat modal.
 *
 * Simplifications vs. the original:
 * 1. Collapsed duplicate effects (scroll ➜ focus) into a single useEffect.
 * 2. Replaced useCallback dep on `messages` with a ref to avoid stale‑closure ESLint warnings and
 *    cut re‑renders.
 * 3. Guarded all window / document access so Replit SSR or tests won't explode.
 * 4. Wrapped localStorage calls in safe helpers – fewer try/catch blocks sprinkled around.
 * 5. Added small helper handlers (toggle, scrollToBooking, makeCall) so JSX stays readable.
 *
 * Design‑wise *nothing* changed – every Tailwind class remains identical so the UI is pixel‑perfect.
 */
export default function AskIan({
  ...
  // 🔄 scroll to latest + focus field when modal opens OR messages add --
  useEffect(() => {
    if (open) {
              ...
              <div className="flex-1 flex flex-col max-w-4xl w-full mx-auto px-2 sm:px-4 pointer-events-auto">
                ...
                <div className="py-6 sm:py-8 space-y-4 sm:space-y-6 px-2 sm:px-0">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-center">
                    ...
                    <OutlineBtn onClick={makeCall}>📞 Call (949) 734-0201</OutlineBtn>
...
function saveHistory(messages: ChatMessage[]): void {
  if (typeof window === 'undefined') return;
  try {
    localStorage.setItem(HISTORY_KEY, JSON.stringify(messages.slice(-10))); // keep last 10
  } catch {
